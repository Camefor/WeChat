    ...
    * */
    public class RContactAdapter : Adapter
    {
        ...
        private void DrawHeadImage(Graphics g, RContact subItem, Rectangle rectSubItem)
        {
            ...
            //if (string.IsNullOrWhiteSpace(subItem.HeadImgUrl))                 //如果头像位空 用默认资源给定的头像
            //    HeadImage = Resources.default_head;
            //else
            //{
            //    string url = Context.base_uri + subItem.HeadImgUrl;

            //    MyDelegate dele = new MyDelegate(AsynLoadImage);
            //    IAsyncResult resf = dele.BeginInvoke(url,new AsyncCallback(Callback),1);
            //    HeadImage = dele.EndInvoke(resf);
        ...
        public delegate Image MyDelegate(string arg);

        private Image AsynLoadImage(string arg)
        {
            Image image = api.HttpTools.GetResponseImage(arg);
            return image;
        }
